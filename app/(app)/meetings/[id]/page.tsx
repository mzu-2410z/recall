...
interface Segment {
  id: string
  speaker: string | null      // DB column name: 'speaker' (not speaker_name)
  start_time_ms: number       // DB column name: 'start_time_ms' (ms, not seconds)
  end_time_ms: number
  text: string
  sequence_num: number
}
...
interface ActionItem {
  id: string
  task: string                // DB column name: 'task' (not text)
  owner?: string | null
...
export default function MeetingDetailPage() {
  ...
  // Use DB field names: transcript_segments, speaker, start_time_ms
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
        ...
        <div className="flex flex-col rounded-xl border border-slate-800 bg-slate-900/60 lg:col-span-5 h-[600px] overflow-hidden">
          ...
          <div className="flex border-b border-slate-800 bg-slate-900/80 p-2 gap-1 overflow-x-auto">
            ...
            >
              <CheckSquare className="h-3.5 w-3.5" /> Action Items ({meeting.action_items?.length || 0})
            </button>
            <button
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {/* TAB 1: Summary — uses meeting_summaries[0] from DB */}
            {activeTab === 'summary' && (
              <div className="space-y-6">
                {!summary ? (
                  <div className="text-center py-8">
                    <p className="text-xs text-slate-400 mb-4">No AI summary yet. Click &quot;Re-summarize AI&quot; to generate one.</p>
                    <button
                      onClick={handleTriggerSummary}
                      disabled={isSummarizing}
                      className="text-xs px-4 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-500 disabled:opacity-50"
                    >
                  ...
                  <>
                    ...
                    {summary.key_takeaways && summary.key_takeaways.length > 0 && (
                      <div>
                        <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Key Takeaways</h3>
                    ...
                    {summary.topics && summary.topics.length > 0 && (
                      <div>
                        <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Topics</h3>
                        <div className="flex flex-wrap gap-2">
                          {summary.topics.map((topic, idx) => (
                            <span key={idx} className="text-[10px] font-medium px-2 py-1 rounded-full bg-slate-800 text-slate-300 border border-slate-700">
                              {topic}
                            </span>
                          ))}
                        </div>
                ...
                <div className="space-y-3">
                  {filteredSegments.length === 0 ? (
                    ...
                    filteredSegments.map(segment => {
                      // start_time_ms is in ms, convert to seconds for video player
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="text-xs font-semibold text-cyan-400">{segment.speaker ?? 'Speaker'}</span>
            ...
            {/* TAB 3: Action Items — uses action_items from DB with 'task' field */}
            {activeTab === 'action-items' && (
              <div className="space-y-3">
                  ...
                  meeting.action_items.map(item => (
                    ...
                    >
                      ...
                      <div className="flex-1 text-xs">
                        <p>{item.task}</p>
                        {item.owner && (
                          <p className="mt-1 text-[10px] text-cyan-400">Owner: {item.owner}</p>
                        )}
                        {item.deadline && (
                          <p className="mt-0.5 text-[10px] text-amber-400">Due: {item.deadline}</p>
                        )}
                      </div>
                    </div>
                  ))
            ...
            {/* TAB 4: Highlights — uses highlights table from DB */}
            {activeTab === 'highlights' && (
              <div className="space-y-3">
                  ...
                  meeting.highlights.map(hl => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-[10px] font-mono text-slate-500">{formatTime((hl.timestamp_ms ?? 0) / 1000)}</span>
                      </div>
                      <p className="text-xs text-slate-300 mt-1">{hl.note ?? ''}</p>
                    </div>
                  ))
                )}
              </div>
